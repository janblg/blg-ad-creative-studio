...
>
> **Next action:** **Phase 4b — product library** (decision #16 part 2): crawl the brand's site for product/inventory pages, extract name + photo into a new `products` table (needs a migration Jan must run), and let the Studio pick a product instead of uploading a photo. THEN Phase 5 node board + whole-app dark design system. THEN Phase 6 review/export, Phase 7 preference memory (`feedback` rows already accumulating from every hook selection).
>
