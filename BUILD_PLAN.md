...
>
> **Phase 3 SHIPPED** (`04a8eb5`, health `2026-08-13-phase3-persistence`, migration **0003 applied by Jan 2026-08-13**). Every Studio step persists: `startBrief` creates the `batches` row (brief/visual_system/master_prompt/ref_asset_ids, product photos recorded as `image_assets` kind `reference`); `approveAndGenerate` stores the base image asset + flips `master_prompt_approved`; `makeHooks` writes all 10 `hooks` rows with §12 metadata (emphasis/visual/why/negative/framework/origin) and returns their ids; `applyHook` creates `creatives` + `image_variants` (composited asset + `layout_spec` JSONB, `is_selected`) and logs a `feedback` row (action `select`); `makeCopy` writes `ad_copy` and links it. `lib/studio/load.ts` rebuilds the feed (`loadBatch`) and lists sessions (`listBatches`); the Studio page reads `?batch=<id>`; the feed has a **Sessions** dropdown + **New session**. All actions authorize via `authorizeBrand`/`authorizeBatch` = RLS + org_id from the row (never from `requireContext()`), per the rule below. Also fixed en route: layout `canvas` is server-injected (model no longer echoes it), studio route `maxDuration` 300 (hook gen exceeded 60s), hook retry only when <3 valid.
>
> **Deliberate constraint:** one session = one ad chain; the composer locks once a brief exists. Per-hook fan-out (decision #4) arrives with the node board.
>
> **Phase 4a SHIPPED** (`d74bafd`, health `2026-08-13-phase4a-website-import`): brand colors + logo import from the client's website. `lib/import/website.ts` (no `server-only` — kept script-testable) fetches the page + up to 4 stylesheets, mines hex/rgb colors ranked by usage, **flags and demotes Bootstrap/Tailwind defaults** (they out-rank real brand colors by raw frequency — verified on werentfun.com where Bootstrap's #0D6EFD/#DC3545 masked the real navy/orange), ranks logo candidates (`img` with "logo" > og:image > apple-touch-icon, SVG/ICO last since sharp often can't decode them), and skips <48px favicons. `/api/brand-import` stores the first decodable logo exactly like a manual upload (alpha preserved via `normalizeLogoToPng`) and asks Claude — **with the logo as vision input** — to assign roles, told to trust the logo over CSS frequency and that `hook_accent` must be bright/saturated for legibility over photos and dark scrims (the JNB #01509B lesson). Colors return as SUGGESTIONS into the settings form for the operator to confirm and Save. Live-verified on two real party-rental sites: werentfun → navy primary / red secondary / **orange accent**; orbitmoonwalks → yellow primary / blue secondary / **red accent**.
>
> **Phases 4b/4c/4d SHIPPED** (`c1564e9`, health `2026-08-13-phase4d-real-product-photos`; migrations **0004, 0005, 0006 applied**). Jan's steer was decisive: clients run on **ERS (Event Rental Systems — assets on files.sysers.com)** and **IO (Inflatable Office)**, so `lib/import/ers.ts` is platform-aware rather than a generic crawler. ERS structure (verified live on orbitmoonwalks.com / werentfun.com): `/` → `/category/<slug>/` → `/items/<slug>/`, all server-rendered. `/api/product-import` walks it and upserts `products` on `(brand_id, source_url)`. **Item names come from the SLUG** — anchors are the CTA ("More Info…") because each card links the item 3×. **Studio now opens with category → 5 products → brief** (`suggestProducts` + `lib/ai/products.ts` picks for SPREAD across size/theme/wet-dry, with a one-line reason each); the chosen product is persisted on the batch and injected into BOTH the prompt engine ("must be the hero") and hook generation; its photo becomes the generation reference so the client's ACTUAL unit appears.
>
> **⚠️ Gotcha #21 — I got this wrong once, publicly:** I concluded ERS renders product photos client-side and told Jan photos were impossible. **False.** The CDN URLs are **protocol-relative** (`//files.sysers.com/...`) and my matcher required `https://`, so it found nothing and I trusted the bug over the browser. Jan checked the sites and corrected me. Always resolve candidate URLs with `new URL(src, base)` before concluding something is absent. Also: cards link `/items/med/<file>` (thumb) while `/items/<file>` is the same image full-size — strip `med/`.
>
> **Phase 5 SHIPPED** (`5db601f`, health `2026-08-14-phase5b-node-board`). **5a — one dark design system app-wide** (decision #12): `globals.css` defines surface/text/accent tokens (`canvas/surface/surface-2/raised/line/line-strong`, `text/text-dim/text-faint`, `accent/accent-2/ok/warn/bad`) + `@custom-variant dark` and the root is FORCED `dark` (a tool for judging images must not flip with the OS); dark scrollbars, `color-scheme: dark` on native controls, shared `panel/field/btn-*/label-caps` utilities; login/brands/brand/settings/studio all restyled and the light ORRISO look retired. *Caution learned: bulk class replacement produced `bg-white text-text` (white-on-white) — audit `grep -rnoE 'bg-white'` after any sweep.* **5b — node board** at `/brands/[id]/board`: React Flow dark dotted canvas, 7 fixed nodes (Product/Brief → Prompt Engine → Image → Hooks → Overlay → Copy → Review) laid out in two rows, custom `PipelineNode` cards with labeled ports, status pill (locked/ready/running/awaiting/done), real parameter rows (no decorative sampler knobs — decision #2) and thumbnails; edges turn green as steps complete and animate while running; session tabs across the top; left conversation column; clicking a node opens `NodeDrawer` where the step is actually worked (editable master prompt, hook picker, product picker, copy). **The graph is DERIVED, never stored** (`lib/studio/graph.ts` `deriveGraph`) — no nodes/edges tables, so freeform wiring stays a UI change. **`useStudioSession` was extracted** so the feed and the board drive ONE pipeline; the chat view stays at `/studio` (linked both ways) until Jan prefers the board.
>
> **Next action:** **Phase 6 — manager review + export**: per-batch review page rendering each creative as a realistic Meta feed preview, role gating on `memberships.role`, comment threads, `changes_requested` routing back to the right step, then ZIP of PNGs + `copy.csv` and a shareable read-only review link (Google Slides was dropped, decision #13). THEN Phase 7 preference memory (`feedback` rows accumulating since Phase 3).
>
