...
>
> **Next action:** **Phase 2d — flyer-style creative design** (decision #15; Jan supplied three reference ads 2026-08-13). Renderer: script accent font (bundled OFL) + per-block `rotateDeg` + `highlight` treatment (brush-stroke background) + per-run `underline`; LayoutSpec schema + vision art-director prompt taught the flyer grammar (stacked mixed-typography lines, one highlighted line, emphasis on the accent). THEN Phase 3 — persistence (batches/hooks/creatives/image_variants/ad_copy/feedback; batch list + resume; RLS rule below; gotcha #8; persist each hook's `VISUAL` line for the per-hook chains). THEN website import (decision #16), THEN Phase 5 node board.
>
