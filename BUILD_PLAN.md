...
>
> **Next action:** **website import** (decision #16: brand colors+logo pull from the brand's site; then a product library pull so the Studio picks a product instead of uploading). THEN Phase 5 node board + whole-app dark design system. THEN Phase 6 review/export, Phase 7 preference memory (the `feedback` rows are already accumulating).
>
